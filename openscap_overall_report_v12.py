#!/usr/bin/env python3
"""
openscap_overall_report_v11.py
==============================
...
Gebruik:
  python openscap_overall_report_v11.py \
      --base-dir  /scans \
...
def parse_score_value(score: str) -> Optional[float]:
...
def score_class(score_value: Optional[float]) -> str:
...
def determine_status(
    ...
    Volgorde:
      ...
      2. COMPLIANT  → geen open findings en geen blocking unknown
      3. LOW RISK   → fail_open ≤ drempel en geen blocking unknown
...
def _categorize_findings(
    ...
    """
    Verwerkt alle rule-resultaten en categoriseert findings.

    Belangrijk in v12:
      - fail/error met ACCEPTED-exception telt niet als open finding.
      - notchecked met ACCEPTED/NOT_APPLICABLE-exception wordt netjes als exception getoond.
      - notchecked zonder exception wordt als NOT_EXECUTED getoond, maar niet als FAIL OPEN.
      - raw NOTCHECKED blijft zichtbaar als informatieve telling, maar blokkeert LOW RISK niet.
      - UNKNOWN wordt alleen blocking als er geen exception voor is.
    """
    ...
    processed_exception_rule_ids: set[str] = set()
    ...
    def make_finding(
        ...
        return Finding(
            ...
            raw_result=raw_result or "unclassified",
    ...
    # Verwerk alle rule-resultaten uit de XML.
    for rr in root.findall(f".//{ns}rule-result"):
        ...
        # Echte failures en scan errors:
        # - met exception: ACCEPTED / NOT_APPLICABLE / etc.
        # - zonder exception: FAIL OPEN
        if res in ("fail", "error"):
            effective_status = (exc or {}).get("status", "FAIL") or "FAIL"
            findings.append(make_finding(rid, title, severity, res, effective_status, exc))
            if exc:
                processed_exception_rule_ids.add(rid)
            continue

        # NOTCHECKED/UNKNOWN:
        # - met ACCEPTED/NOT_APPLICABLE exception: tonen bij findings als exception
        # - zonder exception: tonen als NOT_EXECUTED, maar niet als FAIL OPEN
        if res in ("notchecked", "unknown"):
            if exc and (exc.get("status") in ("ACCEPTED", "NOT_APPLICABLE")):
                findings.append(make_finding(rid, title, severity, res, exc.get("status", "ACCEPTED"), exc))
                processed_exception_rule_ids.add(rid)
            else:
                not_executed_items.append(make_finding(rid, title, severity, res, "NOT_EXECUTED", exc))
                if exc:
                    processed_exception_rule_ids.add(rid)
            continue

    # Verwerk uitzonderingen die niet getriggerd zijn door fail/error/notchecked/unknown.
    for rid, exc in applicable_exceptions.items():
        if rid in processed_exception_rule_ids:
            continue
        ...
        if res == "notselected" or selected == "false":
            not_in_profile_items.append(make_finding(
                rid, title, severity, res or "notselected", "NOT_IN_PROFILE", exc
            ))
        elif res in ("pass", "fixed", "informational", "notapplicable"):
            dormant.append(make_finding(
                rid, title, severity, res, "DORMANT_EXCEPTION", exc
            ))
        elif rid in result_map or rid in rule_defs:
            not_executed_items.append(make_finding(
                rid, title, severity, res or "unclassified", "NOT_EXECUTED", exc
            ))
...
def parse_results_xml(
    ...
    error               = sum(1 for f in findings if f.raw_result == "error" and f.effective_status == "FAIL")
    # UNKNOWN is alleen blocking als deze zonder geldige exception in NOT_EXECUTED staat.
    # Raw NOTCHECKED blijft zichtbaar als aparte telling, maar telt niet als open finding.
    unknown             = sum(1 for f in not_executed_items if f.raw_result == "unknown")
...
def _render_finding_item(f: Finding, badge_label: Optional[str]) -> str:
...
def _render_score_html(score_value: Optional[float], score_text: str) -> str:
...
def _render_counts_grid(item: ServerResult) -> str:
    ...
    rows_html = "".join([
        ...
        _row("Not checked",       item.notchecked,               "dim"),
...
def render_technical_html(
  ...
  <div class="legend">
    ...
    <div class="small" style="margin-top:8px;">
      ...
      <strong>NOT EXECUTED</strong> = rule is niet zinvol uitgevoerd of zonder geaccepteerde exception niet beoordeelbaar. &nbsp;
      <strong>NOTCHECKED raw</strong> = ruwe OpenSCAP-telling; deze blokkeert LOW RISK niet. &nbsp;
      <strong>LOW RISK</strong> = ≤{low_risk_threshold} open findings, geen blocking ERROR/UNKNOWN.
    </div>
  </div>
  ...
  <div class="grid">
    ...
    <div class="card notexecuted"><div class="label">Not executed effectief</div><div class="value">{total_not_exec}</div></div>
    <div class="card"><div class="label">UNKNOWN totaal</div><div class="value">{total_unknown}</div></div>
    <div class="card"><div class="label">NOTCHECKED raw totaal</div><div class="value">{total_notchecked}</div></div>
  </div>
